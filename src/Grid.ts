export class Grid {
    ...
    constructor(
        ...
        this.historyIndex = 0;
        this.cellHistory = Array(width * height)
            .fill(null)
            .map(() => []);
    }

    private getWrappedValue(
        buffer: Float32Array,
        x: number,
        y: number
    ): number {
    ...
    public update(params: {
        ...
        for (let y = 0; y < this.height; y++) {
            for (let x = 0; x < this.width; x++) {
                ...
                spatialBuffer[index] = this.getSmoothNeighborhood(
                    tempBuffer,
                    x,
                    y,
                    spatialSmoothing
        ...
        for (let i = 0; i < this.width * this.height; i++) {
            ...
            const currentContribution =
                spatialBuffer[i] * (1 - regressionWeight);
    ...
    public reset(frameHistory: number): void {
        ...
        this.historyIndex = 0;
        this.cellHistory = Array(this.width * this.height)
            .fill(null)
            .map(() => []);
    }
    ...
    public getImageData(): Uint8ClampedArray {
        ...
        for (let i = 0; i < this.size; i++) {
            ...
            this.valueToColourData(value, data, i * 4);
        }

        return data;
    }

    private valueToColourData(
        value: number,
        ...
        data[pixelIndex + 3] = value * 100 + 145;
    }
}
